...
for index in range(len(jsonInfo["data"]["list"])):
    query_words.append(jsonInfo['data']["list"][index]['query'])
# print(query_words)
...
for gifclass in class_list[:-1]:
    ...
    if ele:
        href_list.append("https://www.soogif.com"+ele.attrs["href"])
# print(href_list)
...
for href in href_list:
    header = {'User-Agent':ua.random}
    now_page = urllib.request.Request(href,headers=header)
    ...
    for title in title_list:
        query_words.append(title.attrs["href"][8:])
# print(query_words)
